""" Создание Question, незавершенные действия, незаполненные поля.
User story -
Docs -
"""
import time
...
from framework.check import check_element_text_is_on_page, check_elem_is_disable, check_elem_is_enable


class TestUnfinishedActions:
    """ Question, unfinished actions.
    """

    def test_question_unfinished_actions_01(self, chrome_browser, valid_cookie_new_user, fake_article_text):
        """ В процессе создания вопроса вернуться в профиль.
    ...
    def test_article_unfinished_actions_02(self, chrome_browser):
        """ Создавая вопрос вписать только Title.
        ...
        - Открыть страницу профиля
        - Убедиться, что в профиле нет опубликованных постов
        - Открыть страницу создания вопроса
        ...
        chrome.write_title()
        check_elem_is_enable(chrome.find_btn_dark())

    def test_article_unfinished_actions_03(self, chrome_browser, fake_question_text):
